...
class SampleListener(Leap.Listener):
    ...
    paintX = int(1200/2)
    paintY = int(695/2)
    paintXPrev = int(1200/2)
    paintYPrev = int(695/2)
    paintZ = False
    ...
    def on_exit(self, controller):
        print ("Exited")
    def indexDistalPosXPrev(self):
        tempX = 0
        if self.paintXPrev< -70 :
            tempX = -70
        elif self.paintXPrev > 150:
            tempX =  150
        else:
            tempX = self.paintXPrev
    ...
    def indexDistalPosY(self):
        ...
        return int(tempY)
    def indexDistalPosYPrev(self):
        tempY = 0
        if self.paintYPrev < 100:
            tempY = 100
        elif self.paintYPrev > 250:
            tempY = 250
        else:
            tempY = self.paintYPrev
                ...
                for b in range(4):
                    ...
                    if b==1:
                        ...
                        while len(tempListX) < 7:
                            tempX = int(bone.next_joint.x)
                            tempY = int(bone.next_joint.y)
                            tempListX.append(tempX)
                            tempListY.append(tempY)
                        self.paintX = sum(tempListX)/6
                        self.paintY = sum(tempListY)/6
