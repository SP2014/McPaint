...
def markerTool(tool, screen, canvas, mx, my, ox, oy, color, thickness): # marker tool
    if tool == "Marker":
...
def eraserTool(tool, screen, canvas, mb, mx, my, ox, oy, thickness): # Eraser tool
    if tool == "Eraser":
        screen.set_clip(canvas)
        if mx == ox and my == oy: #Checks if mouse was stable
...
def sprayTool(tool, screen, canvas, mpos, mb, mx, my, color, thickness): #spray tool
    if tool == "Spray":
        screen.set_clip(canvas)
...
def floodTool(tool, screen, canvas, color, mb, mpos, mx, my): # Flood tool
